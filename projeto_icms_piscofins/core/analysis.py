...
def run_analysis(
    ...
    cond_pis = (merged["bc_icms_final"] > 0) & (merged["bc_pis_pis"] > 0) & (merged["vl_icms_final"] > 0)
    cond_cofins = (merged["bc_icms_final"] > 0) & (merged["bc_cofins_pis"] > 0) & (merged["vl_icms_final"] > 0)

    merged["pis_excluido"] = cond_pis & ((merged["dif_icms_pis"] - merged["vl_icms_final"]).abs() <= tolerancia)
    merged["cofins_excluido"] = cond_cofins & ((merged["dif_icms_cofins"] - merged["vl_icms_final"]).abs() <= tolerancia)
    ...
    merged["status_analise"] = np.select(
        [sem_dados, exclusao, sem_indicio],
        ["Sem dados suficientes", "Exclusão identificada", "Sem indício de exclusão"],
    ...
    report = merged[
        [
            "empresa_pis", "cnpj_matriz_pis", "mes_pis", "ano_pis", "chave_pis", "numero_nota_pis", "serie_pis",
            "item_pis", "cod_produto_pis", "descricao_pis", "cfop", "ind_oper_desc_pis", "valor_item_pis",
            "bc_icms_pis_orig", "vl_icms_pis_orig", "bc_icms_icms_orig", "vl_icms_icms_orig",
            "bc_icms_final", "vl_icms_final", "bc_icms_st_final", "vl_icms_st_final",
            "origem_icms_base", "origem_icms_valor", "bc_pis_pis", "bc_cofins_pis",
            "dif_icms_pis", "dif_icms_cofins", "pis_excluido_txt", "cofins_excluido_txt",
            "status_analise", "motivo", "possui_st", "join_ok", "documento_entrada",
            "credito_pis_estimado", "credito_cofins_estimado", "credito_total_estimado",
        ]
    ].copy()

    report.columns = [
        "Empresa", "CNPJ", "Mês", "Ano", "Chave", "Número da Nota", "Série", "Item", "Código do Produto", "Descrição",
        "CFOP", "Operação", "Valor do Item", "Base de ICMS no PIS", "Valor de ICMS no PIS",
        "Base de ICMS no ICMS/IPI", "Valor de ICMS no ICMS/IPI", "Base de ICMS Final", "Valor de ICMS Final",
        "Base de ICMS ST", "Valor de ICMS ST", "Origem da Base de ICMS", "Origem do Valor de ICMS",
        "Base de PIS Informada", "Base de COFINS Informada", "Diferença ICMS x PIS", "Diferença ICMS x COFINS",
        "Diferença bate com ICMS? PIS", "Diferença bate com ICMS? COFINS", "Status da Análise", "Motivo",
        "Possui ST", "Cruzou com ICMS/IPI", "Documento de Entrada", "Crédito PIS Estimado",
        "Crédito COFINS Estimado", "Crédito Total Estimado",
    ]

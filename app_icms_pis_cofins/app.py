...
st.set_page_config(
    page_title="ICMS na Base PIS/COFINS",
    layout="wide"
)

st.title("Investigação - ICMS na base do PIS/COFINS")
st.caption("Análise ICMS padrão + módulo ICMS-ST preliminar.")
st.warning(
    "Para arquivos grandes, rode localmente. "
    "Esta versão aceita upload de até 1GB via .streamlit/config.toml."
)
...
with st.sidebar:
    ...
    if tipo_analise == "Exclusão ICMS da base PIS/COFINS":
        ...
        st.caption(
            f"PIS: {aliquota_pis:.4%} | "
            f"COFINS: {aliquota_cofins:.4%} | "
        ...
        uf_st = st.selectbox(
            "UF para buscar alíquota ICMS",
            ufs_disponiveis,
            index=ufs_disponiveis.index("PE") if "PE" in ufs_disponiveis else 0,
        )

        col_ini, col_fim = st.columns(2)

        with col_ini:
            data_ini_st = st.date_input(
                "Início",
                value=date(2021, 1, 1),
                format="DD/MM/YYYY",
            )
        ...
        if regime_st == "Alíquota personalizada":
            aliquota_pis_st = st.number_input(
                "Alíquota PIS ST",
                min_value=0.0,
                value=0.0165,
                format="%.4f",
            )
            aliquota_cofins_st = st.number_input(
                "Alíquota COFINS ST",
                min_value=0.0,
                value=0.0760,
                format="%.4f",
        ...
        st.caption(
            f"PIS: {aliquota_pis_st:.4%} | "
            f"COFINS: {aliquota_cofins_st:.4%} | "
...
if tipo_analise == "ICMS-ST - análise preliminar":
    ...
    st.info(
        "Este módulo é preliminar e estimativo. "
        "Ele filtra CFOP 5405, CST PIS/COFINS 01, "
        "valida BC PIS contra valor da operação menos desconto "
        "e calcula crédito estimado por mês/ano."
    )
    ...
    val_pis_st = validate_sheet_exists(
        xls_pis_st,
        required_pis_st,
        "SPED PIS/COFINS - ICMS-ST",
    )
    ...
    if st.button("Processar análise preliminar ICMS-ST", type="primary"):
        try:
            ...
            with st.expander("Visualizar analítico ICMS-ST"):
                analitico = resultado_st.get("02_analitico_documental")
...
with col1:
    arq_icms = st.file_uploader(
        "Excel SPED ICMS/IPI",
        type=["xlsx", "xlsm", "xls"],
    )

with col2:
    arq_pis = st.file_uploader(
        "Excel SPED PIS/COFINS",
        type=["xlsx", "xlsm", "xls"],
    )
...
val_icms = validate_sheet_exists(
    xls_icms,
    required_icms,
    "SPED ICMS/IPI",
)

val_pis = validate_sheet_exists(
    xls_pis,
    required_pis,
    "SPED PIS/COFINS",
)
...
if st.button("Processar análise", type="primary"):
    try:
        with st.spinner("Processando arquivos..."):
            ...
            comparativo = (
                comparativo_c170_c175(cruz_c170, cruz_c175)
                if modo == "C170 + C175"
                else pd.DataFrame()
            )
                ...
                tmp = df[
                    df["STATUS"].isin(
                        [
                            ...
                            "SEM PIS/COFINS",
                            "SEM ICMS/IPI",
                            "ANOMALIA / OUTRAS DEDUÇÕES",
                            "DOCUMENTO NÃO REGULAR",
                            "FORA CST TRIBUTADO",
                        ]
                    )
                ].copy()
            ...
            divergencias = (
                pd.concat(divergencias_lista, ignore_index=True)
            ...
            parametros = pd.DataFrame(
                [
                    {
                        "PARAMETRO": "Data da análise",
                        "VALOR": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                    },
                    {
                        "PARAMETRO": "Modo selecionado",
                        "VALOR": modo,
                    },
                    {
                        "PARAMETRO": "Tolerância",
                        "VALOR": tolerancia,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "VALOR": (
                            "C100 como âncora documental; COD_SIT considerado; "
                            "C170 ICMS/IPI item a item quando disponível; "
                            "C190 mantido como fallback/consolidado."
                        ),
                    },
                    {
                        "PARAMETRO": "Metodologia aba 07",
                        "VALOR": (
                            "Crédito elegível = soma de CREDITO_PISCOFINS_BASE_ESPERADA "
                            "da aba de cruzamento, filtrando CFOP 5102 + CST ICMS 000 + "

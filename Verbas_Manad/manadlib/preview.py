...
def ler_catalogo_k150(path_k150: Path) -> pd.DataFrame:
    ...
    df = df.sort_values(["DESC_RUBRICA", "COD_RUBRICA"], kind="stable").reset_index(drop=True)
    return df


def alertas_descricoes_repetidas(df_rubricas: pd.DataFrame, selected_codigos: Set[str]) -> Optional[pd.DataFrame]:
    """
    Retorna DF com descrições que possuem múltiplos códigos (no universo selecionado ou no catálogo todo).
    """
    ...
    # foca nas selecionadas, se houver
    if selected_codigos:
...
def gerar_previa_k300(
    ...
    # df totais por competência
    rows_c = []
    for comp, total in totais_comp.items():
        rows_c.append(
            {"DT_COMP": comp, "TOTAL": float(total), "QTD LINHAS": int(qtd_comp.get(comp, 0))}
        )
    df_totais_c = pd.DataFrame(rows_c)
    if not df_totais_c.empty:
        # ordena por mmaaaa -> ordenação numérica simples
    ...
    # df sem movimento
    sem_mov_rows = []
    for cod in sorted(rubricas_sem_mov):
        sem_mov_rows.append({"COD_RUBR": cod, "DESCRIÇÃO": desc_map.get(cod, "")})
